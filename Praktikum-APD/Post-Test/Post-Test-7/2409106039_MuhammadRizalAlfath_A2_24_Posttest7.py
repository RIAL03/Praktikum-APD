...
def tampilkan_character(item, key):
    for name, char in item:
        index = list(key).index(name)
        print(index)
        print(f"\n======== Character {index + 1} ========")
...
# fungsi rekursif
def hapus_character(list, nama, index=0):
    if index == len(list):
        print("Character tidak ditemukan")
        return
        
    nama_sekarang, _ = list[index]
    if nama_sekarang == nama:
        del characters[nama]
        del statistiks[nama]
        print(f"\nCharacter {nama} berhasil dihapus")
    else:
        hapus_character(list, nama, index + 1)
        ...
        if akses == True:
            ...
            if pilihan == "1":
                if not characters:
                    ...
                    tampilkan_character(characters.items(),characters.keys())
                ...
                hapus_character(list(characters.items()), name)
